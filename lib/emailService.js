...
export async function sendTicketCreatedEmail(ticket) {
  const adminEmail = process.env.ADMIN_EMAIL || 'admin@brandstored.com';
  
  try {
    // Email to customer
    await transporter.sendMail({
      from: `"Brandstored Support" <${process.env.SMTP_USER}>`,
      to: ticket.userEmail,
      ...
      html: `
        <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
          ...
          <p style="margin-top: 20px; color: #6b7280; font-size: 14px;">
            Review this ticket from your support workflow or data store.
          </p>
        </div>
      `
    });
      ...
      await transporter.sendMail({
        ...
        html: `
          <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
            ...
            <p style="margin-top: 20px; color: #6b7280; font-size: 14px;">
              Review this reply from your support workflow or data store.
            </p>
          </div>
        `
      });
