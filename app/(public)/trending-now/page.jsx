...
export default function TrendingNowPage() {
  ...
  const trendingProducts = useMemo(() => {
    return (products || []).filter(matchesHotDealBadge);
  }, [products]);
  ...
  const filteredAndSortedProducts = useMemo(() => {
    const filtered = applyFilters(trendingProducts);
    return sortProducts(filtered);
  }, [trendingProducts, applyFilters, sortProducts]);
          ...
          <div className="flex-1">
            {filteredAndSortedProducts.length === 0 ? (
              <div className="text-center py-16 bg-white rounded-lg border border-gray-200">
                <p className="text-gray-500 text-lg">No trending products match your filters.</p>
                <button
